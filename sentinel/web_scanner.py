        ...
        try:
            ...
            with socket.create_connection((hostname, port), timeout=10) as sock:
                with context.wrap_socket(sock, server_hostname=hostname) as ssock:
                    ...
                    if cipher and any(
                        weak in cipher[0].lower() for weak in ["rc4", "des", "md5", "null"]
                    ):
                        vuln = WebVulnerability(
                ...
                async with self.session.get(url, allow_redirects=True) as response:
                    ...
                    if response.content_type and "text/html" in response.content_type:
                        ...
                        for form in soup.find_all("form"):
                            ...
                            if (
                                form_info["method"].upper() == "POST"
                                and not form_info["has_csrf_token"]
                            ):
                                vuln = WebVulnerability(
                                    vuln_type="CSRF",
                                    severity="MEDIUM",
    ...
    def _analyze_form(self, form, page_url: str) -> dict[str, Any]:
        ...
        for input_tag in form.find_all(["input", "textarea", "select"]):
            ...
            if input_type == "hidden" and any(
                token in input_name.lower() for token in ["csrf", "token", "_token", "authenticity"]
            ):
